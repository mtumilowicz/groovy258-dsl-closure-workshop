...
class Step7_FsmAnswerTest extends Specification {

    def 'create empty fsm'() {

        when:
        def fsm = Step7_FsmAnswer.create {
        }

        then:
        fsm.initial == new Step1_StateAnswer()
    ...
    def 'create fsm with empty transition'() {
        ...
        fsm.transitions[new Step3_EventAnswer()] == Step2_StateFlowAnswer.of(null, null)
    }
    ...
    def "create fsm with initial state and two transitions"() {
        ...
        then:
        fsm.initial == new Step1_StateAnswer(_state0)
        fsm.transitions.size() == 2
        fsm[_event1] == Step2_StateFlowAnswer.of(_state1, _state2)
        fsm[_event2] == Step2_StateFlowAnswer.of(_state2, _state3)
    ...
    def "order of initialState and add is not important"() {
        ...
        def fsm1 = Step7_FsmAnswer.create {
            add { on _event2 from _state2 into _state3 }
            initialState _state0
            add { on _event1 from _state1 into _state2 }
        }
        and: 'initialState is third'
        def fsm2 = Step7_FsmAnswer.create {
            add { on _event1 from _state1 into _state2 }
            add { on _event2 from _state2 into _state3 }
            initialState _state0
        }
    ...
    def "order of on-from-into inside add is not important"() {
        ...
        then: 'outcome is always the same'
        fsm[_event1] == Step2_StateFlowAnswer.of(_event1From, _event1Into)
        fsm[_event2] == Step2_StateFlowAnswer.of(_event2From, _event2Into)
        fsm[_event3] == Step2_StateFlowAnswer.of(_event3From, _event3Into)
        fsm[_event4] == Step2_StateFlowAnswer.of(_event4From, _event4Into)
    ...
    def "if event is not defined in transitions, stay in current state"() {
        ...
        then:
        fsm.state == new Step1_StateAnswer(_initialState)
    }
    ...
    def "if transition is not designed for current state - stay in current state"() {
        ...
        fsm.state == new Step1_StateAnswer(_state)
    }
    ...
    def "if we are in any state then returning to initial state means returning copy with current state set to initial state"() {
        ...
        with(fsmInInitialState) {
            initial == new Step1_StateAnswer('initialState')
    ...
    def 'when operation is illegal according to fsm specification - error'() {
        ...
        Step7_FsmAnswer.create {
            wrongName
        }

        then:
        Step9_InvalidFsmSpecOperationAnswer ex = thrown()
    ...
    def 'when operation is illegal according to transition specification - error'() {
        ...
        then:
        Step6_InvalidTransitionSpecOperationAnswer ex = thrown()
